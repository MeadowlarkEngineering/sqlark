from query_builder import Delete, Where, PostgresConfig
...
def test_delete_02(pg_connection):
    ...
    Tests sql injection
    """
    s = Delete(table_name="comments").where(column="id", operator="=", value="1; DROP TABLE comments")
    assert s.to_sql(PostgresConfig()).as_string(pg_connection).strip() == \
        'DELETE FROM "comments" WHERE "comments"."id" = %s RETURNING *'
...
def test_delete_03(pg_connection):
    ...
    """
    s = Delete(table_name="comments").where(column="id", operator="=", value=1).where_or(column="author", operator="=", value="John Doe")
    assert s.to_sql(PostgresConfig()).as_string(pg_connection).strip() == \

...
def test_update_02(pg_connection):
    ...
    Tests sql injection
    """
    s = Update(table_name="comments").set(values={"author": "John Doe", "body": "Hello World"}).where(column="id", operator="=", value="1; DROP TABLE comments")
    assert s.to_sql(PostgresConfig()).as_string(pg_connection).strip() == \
        'UPDATE "comments" SET "author"=\'John Doe\',"body"=\'Hello World\'  WHERE "comments"."id" = %s RETURNING *'
...
def test_increment(pg_connection):
    ...
    s = Update(table_name="comments").increment(column="likes", value=1).where(column="id", operator="=", value=1)
    assert s.to_sql(PostgresConfig()).as_string(pg_connection).strip() == \

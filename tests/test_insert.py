...
def test_insert_03(pg_connection):
    ...
    assert s.to_sql(PostgresConfig()).as_string(pg_connection).strip() == 'INSERT INTO "comments" ("author","body") VALUES %s ON CONFLICT ("author") DO UPDATE SET "author" = COALESCE(EXCLUDED."author", "comments"."author"),"body" = COALESCE(EXCLUDED."body", "comments"."body") RETURNING *'
